...
class TSPModel(COMetaModel):
  ...
  def test_step(self, batch, batch_idx, split='test'):
    ...
    for _ in range(self.args.sequential_sampling):
      ...
      np.save(f"/workspace/DIFUSCO/difusco/adj_matrix/adj_matrix_{batch_idx}.npy", adj_mat)
    ...
    # 교수님: DIFUSCO로 디코딩 된 heat map의 각 edge가 각 컬럼에서 몇번째 등수인가? 
    num_points = np_points.shape[0]
    all_edge_distances = np.zeros((num_points, num_points))
    for i in range(num_points):
        for j in range(i + 1, num_points):  # 대칭성으로 인해 i<j인 경우만 계산
            dist = np.linalg.norm(np_points[i] - np_points[j])  # 유클리드 거리 계산
            all_edge_distances[i, j] = dist
            all_edge_distances[j, i] = dist  # 대칭이므로 동일한 거리

    # 엣지 길이에 따라 순위를 매김
    edge_ranks = np.argsort(-all_edge_distances, axis=0) + 1  # 모든 엣지에 대해 내림차순 정렬 후 순위 부여
    
    # flat_edge_distances = all_edge_distances.flatten()
    # edge_ranks = np.argsort(np.argsort(-flat_edge_distances)) + 1  # 내림차순 정렬 후 순위 부여
    # edge_ranks = edge_ranks.reshape((num_points, num_points))
    ...
    for tour in solved_tours:
        for i in range(len(tour) - 1):
            ...
            # 각 엣지의 시작점과 끝점에 해당하는 순위 확인 (flat index 사용)
            edge_rank = edge_ranks[edge_start, edge_end]
            # print("info", edge_start, edge_end, edge_rank)
            self.solved_edge_ranks.append(edge_rank)
  ...
  def on_validation_epoch_start(self):
    self.solved_edge_ranks = []

  def on_validation_epoch_end(self):
    import matplotlib.pyplot as plt
    
    plt.figure()
    plt.hist(self.solved_edge_ranks, bins=50, edgecolor='black', alpha=0.7)
    plt.xlabel('Edge Rank')
    plt.ylabel('Frequency')
    plt.title('Distribution of Edge Ranks for Solved Tour Edges')
    plt.savefig(f'./solved_tour_edge_rank_histogram_1.png')
    plt.show()